...
#' via further `ggplot2::theme(...)` calls.
#'
...
plot.causal_survival_risk <- function(x,
  ...
  p <- ggplot2::ggplot(plot_data, ggplot2::aes(
    ...
    ggplot2::scale_linetype_manual(
      values = stats::setNames(arm_linetypes_v[arms_chr],
                               arm_labels_v[arms_chr])
    ) +
...
build_risk_table_plot <- function(pt_data, id_col, trt_col,
  ...
  ggplot2::ggplot(long, ggplot2::aes(x = .data$k, y = .data$arm)) +
    ggplot2::geom_text(
      ggplot2::aes(label = .data$n, hjust = .data$hjust),
      size  = base_size * 0.32
    ) +
    ggplot2::scale_x_continuous(
      breaks = axis_breaks,
    ...
    ggplot2::theme(
      ...
      axis.ticks.x        = if (show_x_axis) {
        ggplot2::element_line(color = "black", linewidth = 0.3)
      } else {
...
.build_contrast_forest_panel <- function(rows, contrast_type, base_size) {
  ...
  g <- ggplot2::ggplot(rows, ggplot2::aes(y = .data$method)) +
    ...
    ggplot2::geom_point(
      ggplot2::aes(x = .data$estimate, size = .data$prec),
      shape = 15, color = "black"
    ) +
    ggplot2::scale_size_continuous(range = c(3, 6), guide = "none") +
    ggplot2::scale_y_discrete(limits = rev) +
    ggplot2::labs(x = axis_label, y = NULL, title = panel_title) +
    .causal_survival_theme(base_size = base_size) +
    ggplot2::theme(
      plot.title         = ggplot2::element_text(
  ...
  if (contrast_type == "ratio") {
    g + ggplot2::scale_x_log10(limits = x_lim)
  } else {
    g + ggplot2::scale_x_continuous(limits = x_lim)
  }
}
